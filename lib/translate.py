...
import time
import re
from data_iterator import dataIterator_valid
...
def gen_sample(f_init, f_next, x, options, trng=None, k=1, maxlen=30,
    ...
    next_alpha_past = 0.0 * \
        numpy.ones((1, int(SeqL))).astype('float32')  # start position
...
def main(model, dictionary_target, source_fea, source_latex, saveto, wer_file, k=5):
    ...
    # print('worddicts are {}'.format(worddicts))
    ...
    for x, y in valid:
        for xx in x:
            print('%d : %s' %
                  (valid_count_idx+1, valid_uid_list[valid_count_idx]))
    ...
    os.system('python compute-wer.py ' + saveto +
              ' ' + source_latex + ' ' + wer_file)
    fpp = open(wer_file)
    stuff = fpp.readlines()
    fpp.close()
    m = re.search('WER (.*)\n', stuff[0])
